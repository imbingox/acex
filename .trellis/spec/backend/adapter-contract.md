...
- Binance 当前由 coordinator 以 `account.venues.binance.riskPollIntervalMs`（默认 5s）调度，调用 `/papi/v1/account` + `/papi/v1/um/positionRisk`，刷新 `risk.netEquity`、`risk.riskEquity`、`risk.riskRatio`、`risk.riskLeverage`、margin 字段，以及 position 的 `markPrice` / `unrealizedPnl` / `liquidationPrice` 等字段。
