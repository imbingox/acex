...
- `onNoQuote(update)`：仅用于交易所明确推送“当前没有完整双边可成交 quote”的 status-only 输入，例如 Deribit `quote.<instrument>` 缺 bid/ask 任一价格或数量、字段非有限、或 size 非正。该回调不得 resolve 首包 ready，不得发布半成品 `L1Book`；manager 只更新 freshness/status，已有完整 L1 时保留 last complete 顶层价格、时间戳和 version。
...
- **终态 order update 应带 `orderId`**：`RawOrderUpdate.orderId` / `.clientOrderId` 类型上都可选，但 manager 用 `(symbol, orderId)` 作为终态订单（filled / canceled / rejected / expired）的稳定主键（`clientOrderId` 仅 open 内唯一、终态后可复用，不能作终态主键，详见 `order-execution.md`）。adapter 在订单进入终态时**应当**带上交易所 `orderId`；Binance 的 REST 响应、`/papi/v1/um/openOrders`、`/papi/v1/margin/openOrders`、UM `ORDER_TRADE_UPDATE` 与 margin `executionReport` 均满足。只带 `clientOrderId` 的终态单会被 manager 以 provisional key 暂存，`orderId` 与 `clientOrderId` 都缺失时会被丢弃并告警。
...
- **logical stream 契约**：
  ...
  - `status` route：只分发给 `callbacks.onStatus`，不清首包 timer、不 resolve `ready`、不把订阅 freshness 切成 `fresh`。用于 Deribit `no_quote` 这类 status-only 输入；Binance 当前仍只使用 `data` / `ack` / `ignore`。
