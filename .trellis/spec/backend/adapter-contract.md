...
2. 如果只是类型占位，不要加入 runtime adapter registry；保持 `runtimeStatus = "type_only"`，避免 public capability 误报可用。
3. 交易所 payload、签名、URL、rate-limit header、symbol encode/decode 必须放在 `src/adapters/<venue>/`；manager/runtime 不写交易所特判。
4. `MarketAdapter` 首版至少保证 `loadMarkets()` 返回标准 `MarketDefinition[]`，market 数值字段是 canonical decimal string，交易所原始字段只放 `raw`。实现行情流时走 `SubscriptionMultiplexer` + `VenueStreamProtocol`。
5. `PrivateUserDataAdapter` 必须先声明 `readOnly`、`notes`、`accountCapabilities`、`orderCapabilities`。如果该 venue 需要 passphrase 或更复杂凭证，应补 per-adapter credential validator，不要把通用 credential helper 扩成交易所特判表。
6. 在 `src/client/runtime.ts` 注册 adapter 后，manager / coordinator 应通过 registry 和 capability 自然分派；如果需要改 manager 才能识别某个 venue，通常说明 adapter contract 或 capability 声明不完整。
7. 新增 public 能力时更新 `docs/api.md` 的下游使用方式和限制；adapter 接入流程、Layer 规则、基础设施 contract 只写在 `.trellis/spec/backend/`，不要在 `docs/` 下新增 architecture / roadmap / venue-integration 文档。
