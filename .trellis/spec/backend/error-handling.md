...
- `details.exchange` 是下游读取交易所拒绝原因的首选字段。MVP 只解析 Binance-style 顶层 JSON object：`{ code, msg }` 或 `{ code, message }`。
...
| 场景 | `AcexError` 行为 |
|---|---|
| 下单/撤单 REST 返回 `{code,msg}` | `ORDER_*_FAILED`，保留 `cause`，填 `details.exchange.code/message` 和 `details.transport` |
| account/order bootstrap 返回 `{code,msg}` | `ACCOUNT_BOOTSTRAP_FAILED` / `ORDER_BOOTSTRAP_FAILED`，保留 `cause`，填 `details.exchange` |
| market catalog/server-time 返回纯文本/HTML | 不填 `details.exchange`，只填 `details.transport.rawBody/status/url` |
| network/timeout/parse 无可结构化交易所 body | 不填 `details.exchange`，保留 `cause` 与可用 transport metadata |
...
- Binance-style `{code,msg}` / `{code,message}` 解析到 `details.exchange`，`code` string 化。
- 未知 JSON、HTML、纯文本、parse/network/timeout 不填 `details.exchange`。
...
效果：保留 error event 语义，同时让直接调用命令的 SDK 用户在 `catch` 中读取 `error.details.exchange?.message`。
