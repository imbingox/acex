...
| [Adapter Contract](./adapter-contract.md) | MarketAdapter / PrivateUserDataAdapter 接口契约、StreamHandle 语义、回调与错误传播规则、共享 HTTP 传输客户端（REST 骨架 / per-call 幂等 / typed TransportError / 错误脱敏）、签名时间 vs freshness 时间分离（可注入 TimeProvider） | Active |
