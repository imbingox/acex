...
- **无 `orderId` 的终态 order update**：若有 `clientOrderId`，以新生成的 `localOrderId` 作 **provisional** closed 订单暂存（拿到 `(symbol, orderId)` 后迁移到同一个 `localOrderId` 下的正式 venue identity）；若 `orderId` 与 `clientOrderId` 都缺，**丢弃并经 `context.publishRuntimeError("order", ...)` 发 warning**。对应 adapter 侧契约见 `adapters.md`「终态更新应带 orderId」。
...
## Scenario: Binance margin partial liability updates
### 1. Scope / Trigger

- Trigger: 修改 `RawLendingBalanceUpdate`、`BalanceSnapshot.lending` 合并逻辑、Binance PAPI margin `liabilityChange` 映射，或新增只包含部分 lending 字段的账户事件时。
- 目标: 允许交易所推送部分负债字段，同时保持 public `LendingBalanceFacet` 的四个基础字段始终是 canonical string。

### 2. Signatures

```ts
// src/adapters/types.ts
interface RawLendingBalanceUpdate {
  supplied?: string;
  borrowed?: string;
  interest?: string;
  netAsset?: string;
...
- Adapter 可以只发本次事件权威提供的 lending 字段；不要为了凑完整结构在 adapter 层伪造 `supplied` / `netAsset`。
- AccountManager 应把 partial lending update 与已有 `BalanceSnapshot.lending` 合并；缺字段用 previous value，previous 也没有时用 `"0"`。
- Binance PAPI margin `liabilityChange.l` 表示当前 total liability，映射到 `borrowed`，不是 delta，不能累加。
- `liabilityChange` 的 interest 字段如存在映射到 `interest`；没有 collateral / supply 来源时保留旧 `supplied` / `netAsset` 或补 `"0"`。
- public `BalanceSnapshot.lending` 输出仍必须满足 `supplied`、`borrowed`、`interest`、`netAsset` 都存在且为 canonical decimal string。

### 4. Validation & Error Matrix

| Condition | Contract |
|---|---|
| Binance `liabilityChange` 只包含 asset + total liability | 更新该 asset 的 `lending.borrowed`，其它 lending 字段 previous/default 补齐 |
| `liabilityChange.l` 连续两次为 `10`、`12` | 最终 borrowed 是 `"12"`，不是 `"22"` |
| partial update 到达前没有该 asset 余额 | 创建该 asset balance，free/used/total 和缺失 lending 字段按 `"0"` 补齐 |
| adapter 提供非 canonical decimal | manager 出口 canonical 化；不得把 BigNumber / number 暴露到 public snapshot |

### 5. Good / Base / Bad Cases

- Good: `liabilityChange` 只发 `{ borrowed: "12", interest: "0.1" }`，由 AccountManager 合并出完整 lending facet。
- Base: 后续 REST bootstrap / reconcile 返回完整余额时覆盖 stale partial 字段。
- Bad: adapter 把 `liabilityChange.l` 当 delta 累加，或为了补 `netAsset` 用本地不完整余额计算出误导性值。

### 6. Tests Required

- Unit/adapter: Binance margin `liabilityChange` 映射为 partial lending update，且不触发常规 REST reconcile。
- Integration/account: partial lending update 能创建/更新 `BalanceSnapshot.lending`，缺字段补 previous/default `"0"`。
- Regression: 连续 liability updates 覆盖 borrowed，不累加。
...
```ts
previous.lending.borrowed = add(previous.lending.borrowed, event.l);
```

问题：`event.l` 是当前 total liability；累加会把负债越算越大。

#### Correct

```ts
balance.lending = {
  supplied: previous?.lending?.supplied ?? "0",
  borrowed: event.l,
  interest: event.interest ?? previous?.lending?.interest ?? "0",
  netAsset: previous?.lending?.netAsset ?? "0",
};
```

效果：只覆盖事件权威字段，同时保持 public lending facet 结构稳定。
