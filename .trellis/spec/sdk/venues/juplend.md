...
- Quantity mapping: REST position `supply` / `borrow` 是对应 token base units。公开数量按 position 内嵌 vault token decimals 转 human amount：`supplied = supply / 10^supplyToken.decimals`；`borrowed = borrow / 10^borrowToken.decimals`。`dustBorrow` 当前不并入公开 debt 数量。
