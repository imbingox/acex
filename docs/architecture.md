...
**当前结构性限制**：`runtime.ts:96-97` 直接 `new BinanceMarketAdapter()` / `new BinancePrivateAdapter()`，没有 adapter registry。接入第二家交易所前必须先引入按 `Venue` 分派的 adapter 注册表，否则 `subscribe*()` 只会走到 Binance。
...
Binance PAPI UM 的 user data stream 通过 `listenKey` 握手，一条 WS 同时推送 `ACCOUNT_UPDATE` 和 `ORDER_TRADE_UPDATE`。Juplend 不是 WS，而是 HTTP polling，但仍复用同一套 account update / status 通道。共享逻辑收敛在 `PrivateSubscriptionCoordinator`，按 `venue` 从 private adapter registry 选择实现。
...
所以每个账户只持有一条 `PrivateSubscriptionRecord`，内部 stream 可以是 Binance WebSocket，也可以是 Juplend polling handle。
...
4. **注册 adapter**：runtime 已有 private adapter registry；新增 private venue 时把 adapter 加到 registry，并确认只读 venue 的 order 命令在 runtime 或 adapter 层显式拒绝。Market 当前仍是单 `MarketAdapter`，新增 market venue 前需要扩成 registry-aware。
5. **测试**
