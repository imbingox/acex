...
**当前结构性边界**：`runtime.ts:105-118` 通过 `marketAdapters` / `privateAdapters` 两张 `Map<Venue, ...>` 注册表分派 adapter；`MarketManagerImpl` 构造时接收 market adapter registry，并在订阅 / catalog 加载路径按 `key.venue` 取对应 adapter。`PrivateSubscriptionCoordinator` 按账户 venue 选择 private adapter，`createOrder` / `cancelOrder` 等命令通过 `getPrivateAdapter(venue)` 路由。当前 market 侧只注册 Binance 一家；新增 market venue 只需把 adapter 加入 `marketAdapters` Map。
...
调用方 ──► client.market.subscribeL1Book(key)
  ...
  MarketManagerImpl.subscribeL1Book
    ...
    ├── getMarketAdapter(key.venue)              registry 分派
    ├── ensureMarketsLoaded(venue) (幂等)        adapter.loadMarkets()
    ...
    adapter.createL1BookStream(market, callbacks, options)
          │
          ▼
    BinanceMarketAdapter → SubscriptionMultiplexer
      (src/adapters/binance/adapter.ts + stream-protocol.ts)
          │
          ▼
    createManagedWebSocket<BinanceStreamMessage> (src/internal/managed-websocket.ts:47)
          │
...
subscribeAccount()
  │
  ├── bootstrapAccount()
  │     ├── wallet 模式: @jup-ag/lend-read.getAllUserPositions(walletAddress)
  │     ├── direct 模式: @jup-ag/lend-read.getPositionByVaultId(vaultId, positionId)
  │     ├── GET lite-api.jup.ag/lend/v1/borrow/vaults  (TTL 1h)
  │     ├── wallet 模式下可选 options.positionId 过滤单个 NFT position
...
- 缺 `options.walletAddress` 且缺 `options.vaultId + options.positionId`：bootstrap 失败，抛 `ACCOUNT_BOOTSTRAP_FAILED`
...
`createManagedWebSocket` 是所有 WS 流的统一入口（`src/internal/managed-websocket.ts:47`）。所有 adapter 都必须走这里，不允许直接 `new WebSocket`（见 [adapter-contract.md §3.9](../.trellis/spec/backend/adapter-contract.md#39-managedwebsocket-复用要求)）。
...
每条 `BinanceMarketDefinition` 带 `family` 字段（adapter 内部使用，对外签名仅 `MarketDefinition`）。行情流经 `SubscriptionMultiplexer` 复用物理连接（见 §7.1.1）：`createL1BookStream` / `createFundingRateStream` 把 `(channel, market)` 描述符交给共享复用器，`BinanceStreamProtocol` 据 `family` + channel 选 base URL（如 usdm l1book 用 `wss://fstream.binance.com/ws`、usdm funding 用 `.../market/ws`、coinm 两者共用 `wss://dstream.binance.com/ws`），并以 `<id>@bookTicker` / `<id>@markPrice` 作为 `SUBSCRIBE` 流名。`createFundingRateStream` 仅支持 `usdm` / `coinm` 永续市场，并把 Binance mark price stream 的 `r/p/i/T/E` 标准化为 `fundingRate/markPrice/indexPrice/nextFundingTime/exchangeTs`。

### 7.1.1 连接多路复用（已实现）

行情流通过 `SubscriptionMultiplexer`（`src/internal/subscription-multiplexer.ts`，venue-agnostic）复用物理连接：同一 `(venue, channel, base URL)` 下的多个 symbol 共用**一条** WebSocket，通过 JSON `SUBSCRIBE` / `UNSUBSCRIBE` 控制帧动态增删，不再每 symbol 一条连接。交易所特定的 base URL / 帧格式 / 消息路由由注入的 `VenueStreamProtocol` 策略收口（Binance 实现见 `src/adapters/binance/stream-protocol.ts`）。

通用核负责：

- 同一 venue / family / base 下的多个 stream 合并到一条 websocket（`connectionKey` 分组）
- 单连接内维护订阅表，支持动态 `SUBSCRIBE` / `UNSUBSCRIBE`（控制帧批量合并 + 限速）
- 断线后重放全部活跃订阅，并按 **per-subscription** 恢复 ready / freshness（单 symbol 静默即判 stale，不依赖连接级 watchdog）
- 兼容 Binance 单连接限制：订阅数达 `maxSubscriptionsPerConnection`（保守 200，官方硬上限 1024）即同 `connectionKey` 开新连接（连接池）；控制帧 `controlFrameMaxPerSec`（保守 5/s；现货上限 5/s、USDⓈ-M 合约 10/s）

契约细节见 [adapter-contract.md §3.10](../.trellis/spec/backend/adapter-contract.md)。
...
4. **注册 adapter**：runtime 的 `marketAdapters` / `privateAdapters` 都是 `Map<Venue, ...>`。新增 venue 时把 adapter 加进对应 Map（`runtime.ts:105-118`），`MarketManagerImpl` 会按 `key.venue` 从 `marketAdapters` 取；只读 venue 的 order 命令需要在 runtime 或 adapter 层显式拒绝（参考 `getPrivateCommandAccount` 对 juplend 的处理）。
5. **测试**
