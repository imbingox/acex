...
勘误（2026-06-02）：`shared.ts:134` 的 `rate_limited` 实现缺口与 `private-adapter.ts:874` 的签名 `Date.now()` 已由 Step 3 消化；`runtime.ts` / `private-subscription-coordinator.ts` / `order-manager.ts:104` / `context.ts:106` 四处 venue 字面量已由 Step 4 capability 化。`AccountRuntimeOptions` 早已按 `binance` / `juplend` venue 命名空间化，§2.3 和上表中“写死”表述是过时锚点，不再代表当前设计问题。
