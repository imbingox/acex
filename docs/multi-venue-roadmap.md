...
> §1–§6 的代码锚点仍以 baseline `8dc5118` 为准；此后已有数项落地，下表为权威进度，**优先于正文旧锚点**，避免被旧表述误导。

| 路线步骤 | 状态 | 落地 / 任务 |
|---|---|---|
| **Step 1+2**（P0-2 行情多 adapter 分派 + P0-1 `ManagedWebSocket` send/重连重放 + 订阅多路复用器） | ✅ 已完成 | 任务 `06-01-market-venue-dispatch-ws-multiplex`；新增 `src/internal/subscription-multiplexer.ts` |
| **§6** 公共数值契约 BigNumber → canonical 十进制 string | ✅ 已完成 | 任务 `06-01-public-decimal-string-contract`（破坏性公共 API，已发 beta） |
| venue capability **查询**面（Step 4 的前置积木） | ✅ 已完成 | 任务 `05-06-venue-capabilities-query`；`src/client/venue-capabilities.ts` + 各 adapter `orderCapabilities`（**仅供查询，尚未用于下单分派判别**） |
| **Step 3** 共享基础设施（REST 骨架 + 错误归一 + timeout/retry · rate limiter · time provider） | 🚧 进行中 | 任务 `06-01-shared-venue-infrastructure`（分支 `feat/shared-venue-infrastructure`） |
| **Step 4** capability 化分派 + per-adapter credential validator + venue runtime options registry + 清 §4 四处 venue 字面量 | ⬜ 未开始 | — |
...
> 注：§2.5（WS ready/freshness 重设计）已随 Step 1+2 一并落地，不再单列。
...
1. **时钟同步 / timestamp provider** — Binance 签名直接 `Date.now()`(`private-adapter.ts:874` ⟂)。OKX/Bybit/Gate 的私有 REST/WS 对时钟漂移敏感。应抽统一 time provider，把签名时间从裸 `Date.now()` 收口为可注入的 `now()`。
   - **修正（2026-06-01，task 06-01-shared-venue-infrastructure）**：原文「含 server-time 校准 / 系统性失败点」过严。决定 step 3 只抽 `TimeProvider` 接口、默认本地时钟；**server-time 校准不作硬性要求**，延后到接第一个时钟敏感的新所（step 5）时按 venue 补——宿主机一般 NTP 同步，Binance `recvWindow` 吸收常规漂移。
