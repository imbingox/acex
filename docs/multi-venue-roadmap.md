...
| "OKX/Bybit/Gate **接不进来**" | ⚠️ **过绝对**。准确说：若遵守 adapter-contract §3.9「所有 WS 必须走 `createManagedWebSocket`」(`adapter-contract.md:168` ✓) 则实现不了 JSON-subscribe 模型；adapter 技术上能绕开契约自己 `new WebSocket`，只是违规。 |
